...
#' curve(dFWE(x, mu = 0.75, sigma = 0.5),
#'   from = 0, to = 3,
...
#' par(mfrow = c(1, 2))
#' curve(pFWE(x, mu = 0.75, sigma = 0.5),
#'   from = 0, to = 3,
#'   col = "red", las = 1,
...
#' plot(
#'   x = qFWE(p, mu = 0.75, sigma = 0.5), y = p, xlab = "Quantile",
#'   las = 1, ylab = "Probability"
#' )
#' curve(pFWE(x, mu = 0.75, sigma = 0.5), from = 0, add = TRUE, col = "red")
#'
#' ## The random function
#' hist(rFWE(n = 1000, mu = 2, sigma = 0.5),
#'   freq = FALSE, xlab = "x",
...
#'
#' ## The Hazard function
#' curve(hFWE(x, mu = 0.75, sigma = 0.5),
#'   from = 0, to = 2,
...
FWE <- function(mu.link = "log", sigma.link = "log") {
  ...
  structure(
    list(
      family = c("FEW", "Flexible Weibull Extension"),
