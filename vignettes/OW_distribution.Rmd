...
legend("topright", legend=c(expression(paste(mu, 
       ...
       bty="n", lwd=2)
```
And the following figure illustrate the regios corresponding to the different hazard shapes:
...
col4 <- magma(18)[10]
col5 <- magma(18)[1]
col6 <- "blue"#magma(18)[5]

par(mfrow=c(1,1), cex.lab=2, cex.axis=2, mar=c(5,5,1,1), mai=c(1,1,0.2,0.2), 
    mgp=c(3.6,1.1,0))
x <- seq(-3.5, 3.5, length.out = 500)
y <- hyp(x)
plot(x, y, type="l", xlim=c(-3,3), ylim=c(-3,3),
     xlab=expression(sigma), ylab=expression(nu), las=1)
# abline(v=1, lwd=2, lty=3, col=1)
# abline(h=1, lwd=2, lty=3, col=1)
...
```{r}
  plot(my_initial_guess,
       ...
       par_plot=list(mar=c(3.1,3.1,1,2.1)))
```
