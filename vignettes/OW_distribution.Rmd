...
Cooray [-@Cooray2006] used the following data provided by Wang (2000) in order to fit an OW distribution through ML estimation:
 
5, 11, 21, 31, 46, 75, 98, 122, 145, 165, 195, 224, 245, 293, 321, 330, 350, 420.

The data above is the time to failure of an electronic device in hours. As an alternative to classical maximum likelihood estimation (MLE), We used the function \verb|gamlss| to fit an only-intercept model in order to estimate parameters of OW distribution without covariates. Using our `initValuesOW_TTT()`, we can obtain an initial guess and the valid region
...
`initValuesOW_TTT()` function detected the Bathtub hazard function, which corresponds to a convex-then-concave shape of total time on test (TTT) plot
